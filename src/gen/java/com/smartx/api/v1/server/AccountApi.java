...
public interface AccountApi  {
    ...
    @ApiOperation(value = "Get account info", tags={ "Account",  })
    @ApiResponses(value = { 
    ...
    public Response getAccount(@QueryParam("address") @NotNull @Pattern(regexp="^(0x)?[0-9a-fA-F]{40}$") String address);
    ...
    @Consumes({ "application/x-www-form-urlencoded" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get account transactions by height.", tags={ "Account",  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "successful operation", response = GetAccountTransactionsResponse.class),
        @ApiResponse(code = 400, message = "bad request", response = ApiHandlerResponse.class) })
    public Response getAccountTransactions(@QueryParam("address") @NotNull @Pattern(regexp="^(0x)?[0-9a-fA-F]{40}$") String address, @QueryParam("height") @NotNull @Pattern(regexp="^\\d+$") String height);
    ...
    @Consumes({ "application/x-www-form-urlencoded" })
    @Produces({ "application/json" })
    @ApiOperation(value = "get address balance", tags={ "Account" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "successful operation", response = GetBalanceResponse.class),
        @ApiResponse(code = 400, message = "bad request", response = ApiHandlerResponse.class) })
    public Response getBalance(@QueryParam("address") @NotNull @Pattern(regexp="^(0x)?[0-9a-fA-F]{40}$") String address);
}
