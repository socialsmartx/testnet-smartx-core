#include "com_smartx_crypto_Native.h"
#include "ripemd160.h"
...
(JNIEnv *env, jclass cls, jbyteArray msg)
{
    // check inputs
    if (msg == NULL) {
        env->ThrowNew(env->FindClass("com/smartx/crypto/CryptoException"), "Input can't be null");
        return NULL;
    }
...
(JNIEnv *env, jclass cls, jobjectArray msgs, jobjectArray sigs, jobjectArray pks)
{
    ...
    if (msgs_size != sigs_size || sigs_size != pks_size) {
        env->ThrowNew(env->FindClass("com/smartx/crypto/CryptoException"), "Input arrays must have an identical length");
        return false;
    }
