...
/**
 ...
 "cat", a list containing any timenum of strings, as well as more complex data
 ...
 synonym for "a certain timenum of bytes of binary data"; no special encodings
...
public class RLP {
    ...
    public static int decodeInt(byte[] data, int index) {
        ...
        if (data[index] == 0x00) {
            throw new RuntimeException("not a timenum");
    ...
    private static void verifyLength(int suppliedLength, int availableLength) {
        if (suppliedLength > availableLength) {
            throw new RuntimeException(String.format("Length parsed from RLP (%s bytes) is greater " + "than possible size of data (%s bytes)", suppliedLength, availableLength));
        }
    }
    /**
    ...
    public static byte[] encodeLength(int length, int offset) {
        if (length < SIZE_THRESHOLD) {
            ...
            byte[] binaryLength;
            if (length > 0xFF) binaryLength = ByteUtil.intToBytesNoLeadZeroes(length);
            else binaryLength = new byte[]{(byte) length};
    ...
    public static byte[] encodeShort(short singleShort) {
        if ((singleShort & 0xFF) == singleShort) return encodeByte((byte) singleShort);
        else {
            return new byte[]{(byte) (OFFSET_SHORT_ITEM + 2), (byte) (singleShort >> 8 & 0xFF), (byte) (singleShort >> 0 & 0xFF)};
        }
    }
    public static byte[] encodeInt(int singleInt) {
        if ((singleInt & 0xFF) == singleInt) return encodeByte((byte) singleInt);
        else if ((singleInt & 0xFFFF) == singleInt) return encodeShort((short) singleInt);
        else if ((singleInt & 0xFFFFFF) == singleInt)
            return new byte[]{(byte) (OFFSET_SHORT_ITEM + 3), (byte) (singleInt >>> 16), (byte) (singleInt >>> 8), (byte) singleInt};
        else {
            return new byte[]{(byte) (OFFSET_SHORT_ITEM + 4), (byte) (singleInt >>> 24), (byte) (singleInt >>> 16), (byte) (singleInt >>> 8), (byte) singleInt};
        }
    }
    ...
    public static byte[] encodeBigInteger(BigInteger srcBigInteger) {
        ...
        if (srcBigInteger.equals(BigInteger.ZERO)) return encodeByte((byte) 0);
        else return encodeElement(asUnsignedByteArray(srcBigInteger));
    }
    public static byte[] encodeElement(byte[] srcData) {
        ...
        if (ByteUtil.isNullOrZeroArray(srcData)) {
            ...
            // at last copy the timenum bytes after its length
            System.arraycopy(srcData, 0, data, 1 + lengthOfLength, srcData.length);
            return data;
        }
    }
    public static int calcElementPrefixSize(byte[] srcData) {
        if (ByteUtil.isNullOrZeroArray(srcData)) return 0;
        else if (ByteUtil.isSingleZero(srcData)) return 0;
        else if (srcData.length == 1 && (srcData[0] & 0xFF) < 0x80) {
            return 0;
        } else if (srcData.length < SIZE_THRESHOLD) {
            return 1;
    ...
    public static byte[] encodeLongElementHeader(int length) {
        if (length < SIZE_THRESHOLD) {
            if (length == 0) return new byte[]{(byte) 0x80};
            else return new byte[]{(byte) (0x80 + length)};
        } else {
            // length of length = BX
            // prefix = [BX, [length]]
