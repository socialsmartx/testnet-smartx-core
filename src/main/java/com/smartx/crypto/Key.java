...
import com.smartx.crypto.ed25519.cache.PublicKeyCache;
...
public class Key {
    ...
    /**
     * Returns the Smartx address.
     */
    public byte[] toAddress() {
        return Hash.h160(getPublicKey());
    }
    /**
     * Returns the Smartx address in {@link String}.
     */
    ...
    /**
     * Signs a message and return base58 string
     *
     * @param message
     *            message
     * @return
    ...
    public String sign(String message) {
        try {
            byte[] sig;
            if (Native.isEnabled()) {
                sig = Native.sign(message.getBytes(), Bytes.merge(sk.getSeed(), sk.getAbyte()));
            } else {
                EdDSAEngine engine = new EdDSAEngine();
                engine.initSign(sk);
                sig = engine.signOneShot(message.getBytes());
            }
            Signature sign = new Signature(sig, pk.getAbyte());
            return Base58.base58Encode(sign.toBytes());
        } catch (InvalidKeyException | SignatureException e) {
            throw new CryptoException(e);
        }
    }
    /**
     * Verifies a signature.ye
     *
     * @param message
     *            message
     * @param signature
    ...
    public static boolean verifyBatch(Collection<byte[]> messages, Collection<Signature> signatures) {
        ...
        return Native.verifyBatch(messages.toArray(new byte[messages.size()][]), signatures.stream().map(Signature::getS).toArray(byte[][]::new), signatures.stream().map(Signature::getA).toArray(byte[][]::new));
    }
    /**
    ...
    public static class Signature {
        ...
        /**
         * Parses from base58 encoded string
         *
         * @param base58Encode
         * @return a {@link Signature} if success,or null
         */
        public static Signature fromBase58(String base58Encode) {
            byte[] bytes = Base58.base58Decode(base58Encode);
            if (bytes == null || bytes.length != LENGTH) {
                return null;
            }
            byte[] s = Arrays.copyOfRange(bytes, 0, S_LEN);
