...
import com.smartx.crypto.ed25519.bip32.util.BytesUtil;
import com.smartx.crypto.ed25519.bip32.util.HashUtil;
import com.smartx.crypto.ed25519.bip32.util.Hmac;
import com.smartx.crypto.ed25519.bip32.util.Secp256k1;
...
public class HdKeyGenerator {
    private static final Logger logger = Logger.getLogger(HdKeyGenerator.class);
    ...
    public HdKeyPair getMasterKeyPairFromSeed(byte[] seed, KeyVersion keyVersion, CoinType coinType) {
        ...
        switch (scheme) {
            ...
            case BIP32_ED25519:
                ...
                byte[] c = Hmac.hmac256(BytesUtil.merge(new byte[]{1}, seed), scheme.getSeed().getBytes(StandardCharsets.UTF_8));
    ...
    public HdKeyPair getChildKeyPair(HdKeyPair parent, long child, boolean isHardened) {
        HdPrivateKey privateKey = new HdPrivateKey();
        HdPublicKey publicKey = new HdPublicKey();
        HdKeyPair key = new HdKeyPair(privateKey, publicKey, parent.getCoinType(), getPath(parent.getPath(), child, isHardened));
        if (isHardened) {
            child += 0x80000000;
        ...
        switch (parent.getCoinType().getScheme()) {
            ...
            {
                //logger.debug("parent, kLP = " + Hex.encode(kLP));
                //logger.debug("parent, kRP = " + Hex.encode(kRP));
                //logger.debug("parent,  AP = " + Hex.encode(AP));
                //logger.debug("parent,  cP = " + Hex.encode(cP));
            }
            BigInteger kLiBI = parseUnsignedLE(ZL).multiply(BigInteger.valueOf(8)).add(parseUnsignedLE(kLP));
            BigInteger order = BigInteger.valueOf(2).pow(252).add(new BigInteger("27742317777372353535851937790883648493"));
            if (kLiBI.mod(order).equals(BigInteger.ZERO)) {
                return null;
            }
            IL = serializeUnsignedLE256(kLiBI);
            BigInteger kRiBI = parseUnsignedLE(ZR).add(parseUnsignedLE(kRP)).mod(BigInteger.valueOf(2).pow(256));
