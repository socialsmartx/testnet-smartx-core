...
/**
 * Smartx crypto native implementation
 */
public class Native {
    private static final Logger logger = Logger.getLogger(Native.class);
        ...
        switch (os) {
            case LINUX:
                if (SystemUtil.getOsArch().equals("aarch64")) {
                    enabled = loadLibrary("/native/Linux-aarch64/libsmartxcrypto.so");
                } else {
                    enabled = loadLibrary("/native/Linux-x86_64/libsmartxcrypto.so");
                }
                break;
            case MACOS:
                enabled = loadLibrary("/native/Darwin-x86_64/libsmartxcrypto.dylib");
                break;
            case WINDOWS:
                enabled = loadLibrary("/native/Windows-x86_64/libsmartxcrypto.dll");
                break;
            default:
                break;
        }
