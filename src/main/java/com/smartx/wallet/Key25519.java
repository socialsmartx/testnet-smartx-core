...
import static com.smartx.util.FileUtil.POSIX_SECURED_PERMISSIONS;
...
import com.smartx.core.state.AccountState;
import com.smartx.crypto.*;
import com.smartx.crypto.ed25519.bip32.CoinType;
import com.smartx.crypto.ed25519.bip32.HdKeyPair;
import com.smartx.crypto.ed25519.bip32.key.KeyVersion;
import com.smartx.crypto.ed25519.bip39.Language;
import com.smartx.crypto.ed25519.bip39.MnemonicGenerator;
import com.smartx.crypto.ed25519.bip44.Bip44;
// import com.smartx.message.CliMessages;
import com.smartx.util.*;

import io.github.novacrypto.base58.Base58;

public class Key25519 {
    private static final Logger logger = Logger.getLogger(Key25519.class);
    private static final int VERSION = 4;
    private static final int SALT_LENGTH = 16;
    private static final int BCRYPT_COST = 12;
    private static final Bip44 BIP_44 = new Bip44();
    private static final int MAX_HD_WALLET_SCAN_AHEAD = 64;
    // the BIP-44 path prefix for smartx addresses
     ...
     */
    public Key25519(File file, String network) {
        this.file = file;
     ...
     */
    protected LinkedHashSet<Key> readAccounts(byte[] key, SimpleDecoder dec, boolean vlq, int version) throws InvalidKeySpecException {
    ...
    /**
     ...
     @param idx account index, starting from 0
     @return
     @throws WalletLockedException
     */
    ...
    /**
     ...
     @param address address bytes of the account to delete
     @return true if the account was successfully deleted, false otherwise
     @throws WalletLockedException
     */
    ...
    /**
     ...
     @return
     */
    public int addWallet(Key25519 w) {
        requireUnlocked();
        // import addresses
    ...
    public static String sign(String strraw, com.smartx.crypto.Key key) {
        //byte[] rawhash = Hash.h256(strraw.getBytes());
        String sigraw = Base58.base58Encode(key.sign(strraw.getBytes()).toBytes());
        return sigraw;
    }
    public static boolean verify2(String hash, String base58RawSig, String address) {
        //byte[] rawhash = Hash.h256(rawstr.getBytes());
        //String base58EncodeHash = Base58.base58Encode(rawhash);
        Key.Signature signature = Key.Signature.fromBase58(base58RawSig);
        if (signature == null) {
            return false;
        }
        if (!Key.verify(hash.getBytes(), signature.toBytes())) return false;
        String signatureAddress = Hex.encode(signature.getAddress());
        return signatureAddress.equals(address);
    }
    public static boolean verify(String base58RawHash, String base58RawSig, String address) {
        byte[] rawhash = Base58.base58Decode(base58RawHash);
        Key.Signature signature = Key.Signature.fromBase58(base58RawSig);
        if (signature == null) {
            return false;
        }
        if (!Key.verify(rawhash, signature.toBytes())) return false;
        String signatureAddress = Hex.encode(signature.getAddress());
        return signatureAddress.equals(address);
    }
}
