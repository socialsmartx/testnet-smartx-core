...
public class PubSub {
    ...
    public static void PubMessage(String cmd, String key, String value) {
        GetHeightEvent event = new GetHeightEvent(cmd);
        event.message.args.put(key, value);
        PubSubFactory.getDefault().publish(event);
    }
    public static void PubMessage(String cmd, Block blk) {
        GetHeightEvent event = new GetHeightEvent(cmd);
        event.message.collection = new MCollection();
        event.message.collection.mblock = blk;
        PubSubFactory.getDefault().publish(event);
    }
    /**
